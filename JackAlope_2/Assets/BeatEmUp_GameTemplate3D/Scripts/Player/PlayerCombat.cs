...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
    ...
    public DamageObject[] Specials; //a list of special attacks
	...
	void OnEnable(){
        ...
        this.Energybar = GameObject.FindGameObjectWithTag("EnergyBar");
	...
	private void CombatInputEvent(INPUTACTION action) {
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			if (action == INPUTACTION.PUNCH && playerState.currentState != UNITSTATE.PUNCH && playerState.currentState != UNITSTATE.KICK && isGrounded) {
                ...
                if (attackNum == 2)
                {
                    if (this.Energybar.GetComponent<GainEnergy>().GetComponent<Slider>().value > 0.15f)
                    {
	...
	public void doAttack(DamageObject d, UNITSTATE state, INPUTACTION inputAction){
    ...
    public void Revived()
    {
        ...
        StartCoroutine(ReviveLevel());
    }

    IEnumerator ReviveLevel()
    {
    ...
    IEnumerator ReStartLevel(){
		...
		if (UI != null){
            ...
            GameObject.FindGameObjectWithTag("GameOver").GetComponent<CountDownGO>().CountDownManual();
        }
	}
}